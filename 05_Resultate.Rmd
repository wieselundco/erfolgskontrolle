...
## Erhebung 3: Erhebung von Asthaufen Eigenschaften zur Ermittlung attraktivitätsfördernder Faktoren


Um den Einfluss der erhobenen Parameter auf die Hermelinbesuche zu prüfen, berechnen wir ein `poisson` basiertes Generalized Linear Model. Dieses Modell zeigt, dass sich das Hermelinvorkommen durch die Anzahl beanachbarter Korridore (schwach signifikant `p = 0.1`, negative Korrelation), vor allem aber durch die Stärke des verwendeten Astmaterials (hoch signifikant, `p = 0.01`, positive Korrelation) erklären lässt. Letzteres Resultat besagt, dass die Nachweisquote der Zielarten bei Asthaufen mit einem hohen Bestandteil an grobem Holz hoch signifikant grösser ist.
Der vollständige Modelloutput ist hier ersichtlich:

```{r}
# asthaufen_qualitaet_df2 %>%
#   mutate(total_nach_weise_wk_2019_2020 = as.integer(total_nach_weise_wk_2019_2020)) %>%
#   glm(total_nach_weise_wk_2019_2020~., data = ., family = "poisson") %>%
#   summary()
...
asthaufen_qualitaet_df3 %>% 
  glm(nachweise_bool~
        astmaterial+volumen+storung+beute_angebot+andere_feinde,
      data=.,
      family="binomial") %>% summary()

asthaufen_qualitaet_df3 %>% #simplified da correlated
  glm(nachweise_bool~
...
#poisson regression
asthaufen_qualitaet_df3 %>% 
  glm(nachweise_int~astmaterial+volumen+storung+beute_angebot+katzen+andere_feinde,data=.,
      family="poisson") %>% summary()

#simplified da correlated
asthaufen_qualitaet_df3 %>% 
  glm(nachweise_int~
...
asthaufen_qualitaet_df3 %>% 
  ...
  summarise(sd=sd(nachweise_int),
            var=var(nachweise_int),
            mean=mean(nachweise_int))
#Die Varianz sollte ungefähr gleich gross sein wie der Mittelwert

asthaufen_qualitaet_df3 %>%
  ggplot(aes(x=nachweise_int)) + 
    geom_histogram() 
#theoretische Verteilung
plot(dpois(1:7,lambda=1.72),type="h")
#sieht ein bisschen nach zero inflation aus
...
chisq.test(
  ...
  p=dpois(1:7,1.72),
  rescale.p = TRUE)
#empirische Verteilung passt nicht gut zur theoretischen Verteilung

#zero-inflation see https://stats.idre.ucla.edu/r/dae/zip/
library(pscl)
summary(m1 <- zeroinfl(nachweise_int~
                         astmaterial+storung+beute_angebot,data=asthaufen_qualitaet_df3))

#ols regression
asthaufen_qualitaet_df3 %>% lm(nachweise_int~
                   astmaterial+volumen+storung+beute_angebot,data=.) -> model1
asthaufen_qualitaet_df3 %>% lm(sqrt(nachweise_int)~
             astmaterial+volumen+storung+beute_angebot,data=.) -> model2
asthaufen_qualitaet_df3 %>% lm(nachweise_bool~
             astmaterial+volumen+storung+beute_angebot,data=.) -> model3
summary(model1)
summary(model2)
summary(model3)
par(mfrow=c(1,3))
...
#statistische Tests, z.B. mit Fishers exaktem Test, ob 
kt = with(asthaufen_qualitaet_df3, table( nachweise_bool , astmaterial ))
kt
fisher.test(kt)
#oder mit den ganzen Daten ohne Zusammenfassung der Klassen
kt = with(asthaufen_qualitaet_df3, table( nachweise_int , astmaterial ))
kt
fisher.test(kt)
#sind so halt zu wenig Daten
#oder mit den ganzen Zahldaten mite Zusammenfassung der Klassen der unabhängigen Variabeln
kt = with(asthaufen_qualitaet_df3, table( nachweise_int , astmaterial ))
kt
fisher.test(kt)
#sind so halt auch zu wenig Daten

#tests der weiteren Variabeln, kann schoener programmiert werden 
#bonferroni-Korrektur, da wir 9-Tests machen
for (variable in tail(names(asthaufen_qualitaet_df3),-1)) {
  print(variable)
  kt = table( asthaufen_qualitaet_df3$nachweise_bool , asthaufen_qualitaet_df3[[variable]] )
  print(kt)
  print(fisher.test(kt))
  print("adjusted p-value")
  print(p.adjust(fisher.test(kt)$p.value,n=9))
}
