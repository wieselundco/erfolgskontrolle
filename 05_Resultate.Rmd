...
Um den Einfluss der erhobenen Parameter auf die Hermelinbesuche zu prüfen, berechnen wir ein `poisson` basiertes Generalized Linear Model. Dieses Modell zeigt, dass sich das Hermelin vorkommen durch die Anzahl beanachbarten Kleinstrukturen (schwach signifikant `p = 0.1`, negative Korrelation), vor allem aber durch das verwendete Astmaterial (hoch signifikant, `p = 0.01`, positive Korrelation) erklären lässt. Der vollständige Modelloutput ist hier ersichtlich:
