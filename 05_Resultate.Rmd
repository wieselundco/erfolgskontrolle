...
asthaufen_qualitaet_df2 <- asthaufen_qualitaet_df %>%
    ...
    imap(function(x, y){
      ...
      class1 <- levels(x)[2]
      class2 <- levels(x)[4]
      newclasses <- ifelse(as.integer(x) > class_border,class1, class2)
...
Da es sich um Zähldaten handelt, berechnen wir ein Generalized Linear Model der Famillie `poission` wo wir die Anzahl Hermelinbesuche durch die erhobenen Parameter zu erklären versuchen. Dabei kommt heraus, dass sich das Hermelin vorkommen durch das verwendete Astmaterial (hoch signifikant, `p = 0.01`, `estimate = 1.46995`) und die Anzahl beanchbarten Kleinstrukturen (schwach signifikant `p = 0.1`, `estimate = -0.59077`) erklären lässt. Der vollständige Modelloutput ist hier ersichtlich:
...
#   cowplot::plot_grid(plotlist = .)
# 
#   ggsave("images/asthaufen_qualitaet_histogramm.png", width = 25, height = 12, units = "cm")
# } else{
#   knitr::include_graphics("images/asthaufen_qualitaet_histogramm.png")
# }
# 
