...
if(run_full){
  ...
  asthaufen_monitoring <- read_csv("input_sensitive/Standorte_Monitoring_Asthaufen_2019-2020.csv") %>%
    transmute(struktur_id = spurentunnel_nummer, monitoring = TRUE, monitoring_typ = "spurentunnel")
  
  winterquartier_monitoring <- read_csv("input_sensitive/winterquartiere.csv") %>%
    transmute(struktur_id, monitoring = TRUE, monitoring_typ = "fotofalle")
  
  monitoring <- rbind(asthaufen_monitoring, winterquartier_monitoring) 
  
  strukturen_alle <- strukturen_alle %>%
    left_join(monitoring, by = "struktur_id") %>%
    mutate(monitoring = !is.na(monitoring)) %>%
    arrange((monitoring))

  strukturen_alle %>%
    filter(monitoring) %>%
    select(struktur_id, struktur_typ, monitoring_typ) %>%
    write_sf("images/spurentunnel_fotofallen_standorte.gpkg")
